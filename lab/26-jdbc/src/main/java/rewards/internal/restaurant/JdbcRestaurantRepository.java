...
public class JdbcRestaurantRepository implements RestaurantRepository {
	...
	public Restaurant findByMerchantNumber(String merchantNumber) {
		...
		Restaurant restaurant = jdbcTemplate.queryForObject(sql,
				(resultSet, rowNum) -> new Restaurant(resultSet.getString("MERCHANT_NUMBER"), resultSet.getString("NAME")), // RowMapper
				merchantNumber); // le(s) paramètre(s)

		// OU
		Restaurant restaurant2 = jdbcTemplate.queryForObject(sql,
				(resultSet, rowNum) -> { // RowMapper
					Restaurant restaurantTemp = new Restaurant(resultSet.getString("MERCHANT_NUMBER"), resultSet.getString("NAME"));
					restaurantTemp.setBenefitPercentage(new Percentage(resultSet.getObject("BENEFIT_PERCENTAGE", BigDecimal.class)));
					restaurantTemp.setBenefitAvailabilityPolicy(mapBenefitAvailabilityPolicy(resultSet)); // (déjà implémenté)
					return restaurantTemp;
				},
				merchantNumber); // le(s) paramètre(s)

		// OU
		Restaurant restaurant3 = jdbcTemplate.queryForObject(sql,
				(resultSet, rowNum) -> mapRestaurant(resultSet), // RowMapper (déjà implémenté)
				merchantNumber); // le(s) paramètre(s)
